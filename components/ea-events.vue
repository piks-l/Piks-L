  ...
  <div id="main-container">
    <div id="parallax" data-image-src="https://electrobotik.netlify.com/images/crop.jpg">
      <div  class="container scene_element scene_element--fadein padding">
        ...
        <div class="grid">
            <div v-for="lieu in lieux" :key="lieu.date" class="element-item">
                 <nuxt-link :to="lieu._path+'/'">
                            <img class="lazy" v-lazy="lieu.thumbnail" :alt="artiste.title">
                            <h3 class="name small">{{lieu.title}}</h3>
                            <a target="_blank" v-for="socia in events" :key="socia.events">{{socia.events}}</a>
                </nuxt-link>
            </div>
        </div>
      </div>
